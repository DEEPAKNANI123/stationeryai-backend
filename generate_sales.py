...
# Add extra random products to make list bigger (100+)
for i in range(1, 51):
...
# Insert Products if DB empty
# 🔥 Delete old products and insert again
...
db.commit()
print("✅ Products re-inserted successfully!")

# Generate huge sales data
TOTAL_RECORDS = 100000  # 🔥 change this to 200000 if needed

for i in range(TOTAL_RECORDS):
    name, cat, price = random.choice(products_list)
    qty = random.randint(1, 10)

    total_amount = qty * price
    profit = total_amount * random.uniform(0.1, 0.35)
    ...
    # commit every 5000 rows for speed
    if i % 5000 == 0:
        db.commit()
...
print(f"✅ {TOTAL_RECORDS} sales records inserted successfully!")
